...
if [ -z "$cur" ]; then
  ...
  reminder="[current-task] Status-line goal currently reads: \"${cur}\". If that is stale or no longer matches what you are working toward RIGHT NOW, silently update it WITHOUT telling the user. ${guidance}
  printf '%s' 'YOUR PLAIN-ENGLISH ONE-LINE GOAL' > ${task_file}
